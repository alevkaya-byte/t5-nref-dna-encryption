# -*- coding: utf-8 -*-
"""
Created on Tue Jun 23 09:56:01 2026
...
# -*- coding: utf-8 -*-
"""
BMC Bioinformatics – T5 multi-stream independence analysis.
...
bağımsız akış çifti karşılaştırılır.

Her koşum klasöründeki temel girdiler:

    *.dna.txt   -> 500.000 DNA bazı
    *.bits.txt  -> 1.000.000 bit

Kod önce aynı kök ada sahip:

    *.json
    *.dna.txt
    *.bits.txt

paketini otomatik bulur.

Bu nedenle uzun seed ve tarih içeren dosya adlarının elle yazılması
gerekmez.
...
Yorum:
- Bağımsız ve dengeli iki bit akışı için beklenen Hamming oranı ≈ 0.50.
- Uniform ve bağımsız iki DNA akışı için beklenen mismatch oranı ≈ 0.75.
- DNA kompozisyonları tam uniform değilse, iki akış için düzeltilmiş beklenti:

      1 - Σ p_i(b) p_j(b)

  olarak ayrıca hesaplanır.

Bu analiz bir multi-stream independence tanılamasıdır.
Plaintext avalanche testi veya biçimsel kriptografik güvenlik kanıtı değildir.
"""
